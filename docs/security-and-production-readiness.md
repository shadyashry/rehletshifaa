...
- **Contact verification (OTP) is not legal identity** and never mutates the operational/clinical case status. A verified OTP stamps only the channel that carried it (`WHATSAPP` → `phone_verified_at`, `EMAIL` → `email_verified_at`), preserving any existing timestamps; account activation links the subject only and sets **no** verification timestamp. The patient may choose WhatsApp or email per challenge, but the destination is always the on-file contact — never caller-supplied — and only masked destinations are exposed.
- **Patient conversion layer.** Every onboarding/identity/readiness endpoint enforces object-level patient/representative ownership. Legal identity verification stores minimum-necessary data only — legal name and date of birth are `CryptoService`-encrypted, the document reference is masked, and **no biometric content or full document is stored**; a genuine external provider sits behind `IdentityVerificationPort` (only a profile-restricted simulator and authorized manual review ship). Manual identity review is limited to the narrowly-scoped `PATIENT_IDENTITY_REVIEWER` role (never `CREDENTIALING_ADMIN`) and requires recent authentication, an explicit reason, and an audited immutable outcome. Deposit waivers require an authorized Finance/System-Admin action with recent authentication and a mandatory reason (no silent coordinator waiver) and preserve the append-only `payment_events` ledger. Customer readiness is computed only by the backend and enforced server-side before any non-cancellable commitment, returning structured blocking reasons. Coordinators may monitor and request information but cannot declare identity verified, complete onboarding, record payment, waive a deposit, or enter a patient's OTP.
