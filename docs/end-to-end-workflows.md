...
**Entering `TRAVEL_COORDINATION`:** the case lifecycle and its entry invariants live in one place, `CaseTransitionPolicy`, and every path that moves a case validates there (coordinator transitions, dedicated operations, the public proposal decision, the deposit handoff). `ACCEPTED → TRAVEL_COORDINATION` requires an accepted proposal, a settled deposit where one is due, and no profile step still owed by the patient (activation, contact verification, consents, onboarding; identity is the later travel-confirmation gate). The move is made only by `CaseHandoffService` on a `DepositSettled` or `PatientReadinessChanged` event — a travel-plan save is data entry and never changes the stage, and the coordinator's transition endpoint refuses the target (`DEDICATED_OPERATION_REQUIRED`). A deposit settled before the profile is active leaves the case in `ACCEPTED`; the next readiness event brings it across, exactly once. The macro transition `ACCEPTED → TRAVEL_COORDINATION` is **not** overloaded with the gate.
