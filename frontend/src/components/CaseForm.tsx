...
export function CaseForm({ locale, d }: { locale: Locale; d: Dictionary }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const createResponse = await fetch(`${apiBase}/api/v1/cases`, { method: "POST", headers: { "Content-Type": "application/json", "X-Request-ID": crypto.randomUUID() }, body: JSON.stringify({ ...result.data, preferredLanguage: locale, turnstileToken }) });
