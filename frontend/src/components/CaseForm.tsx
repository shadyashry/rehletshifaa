...
import { CheckCircle2, FileUp, LockKeyhole, ChevronDown, Search, Check, X } from "lucide-react";
...
export function CaseForm({ locale, d }: { locale: Locale; d: Dictionary }) {
  ...
  function onFiles(next: File[]) { begin(); setFiles(next); }
  ...
  return <>
    ...
    <form className="card p-6 md:p-9" onSubmit={submit} noValidate>
      ...
      <fieldset className="mt-9 min-w-0 border-0 p-0">
        ...
        <div className="mt-6"><span className="mb-2 block text-sm font-bold text-ink-800">{d.form.files} <span className="font-normal text-ink-400">({d.form.optional})</span></span><label className="flex cursor-pointer flex-col items-center rounded-lg border border-dashed border-line-strong bg-brand-50 px-5 py-8 text-center hover:border-brand-600"><FileUp className="text-accent-700" /><span className="mt-3 font-bold text-brand-700">{d.form.choose}</span>{files.length > 0 && <span className="mt-2 text-sm text-ink-500">{files.length} {d.form.selected}</span>}<input className="sr-only" type="file" multiple accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" onChange={e => { onFiles(Array.from(e.target.files ?? [])); touch("files"); }} /></label>{fieldError("files") && <p className="error-text mt-2">{fieldError("files")}</p>}<p className="mt-3 text-sm leading-6 text-ink-500">{d.form.uploadHelp}</p></div>
      </fieldset>
