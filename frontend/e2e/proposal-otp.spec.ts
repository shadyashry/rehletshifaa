...
test("patient verifies with OTP before seeing pricing, then accepts", async ({ page }) => {
  ...
  let decided = ""; let acknowledged: boolean | undefined;
  await page.route(`${BASE}/${TOKEN}/decision`, async (route) => { const body = JSON.parse(route.request().postData() ?? "{}"); decided = body.decision; acknowledged = body.acknowledgementAccepted; await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ id: "v1", status: "ACCEPTED" }) }); });
  ...
  // The acknowledgement gates the continue path, and the server is told about it explicitly.
  await page.getByRole("checkbox").check();
  await page.getByRole("button", { name: "Acknowledge & continue" }).first().click();
  await expect(page.getByRole("heading", { name: "your estimate is acknowledged" })).toBeVisible();
  expect(decided).toBe("ACKNOWLEDGED");
