...
test("patient can submit the minimal case and see a case number", async ({ page }) => {
  ...
  await page.route(`http://localhost:8080/api/v1/cases/${caseId}/submit`, route => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ caseNumber: "RS-2026-000001", status: "RECEIVED", statusToken: "status-token" }) }));
  await page.goto("/en");
