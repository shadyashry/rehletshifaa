...
class DocumentServiceTest {
    ...
    @BeforeEach void setup(){var documents=mock(MedicalDocumentRepository.class);cases=mock(CaseService.class);var storage=mock(StoragePort.class);var inspector=mock(DocumentInspectionPort.class);when(storage.presign(anyString(),anyString(),anyLong())).thenReturn(new StoragePort.PresignedUpload("https://upload.invalid",Map.of(),300));service=new DocumentService(documents,cases,storage,inspector,Clock.fixed(Instant.parse("2026-08-27T00:00:00Z"),ZoneOffset.UTC),15*1024*1024,100*1024*1024,25,"application/pdf,image/jpeg,image/png");when(cases.findDraft(any())).thenReturn(new MedicalCase(UUID.randomUUID(),"RS-2026-000001","Jane Doe","Kenya","+254700000000",null,"en",Instant.now()));}
