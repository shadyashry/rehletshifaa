...
class CaseServiceTest {
    ...
    @BeforeEach void setUp(){repository=mock(MedicalCaseRepository.class);intake=mock(IntakeLifecycleService.class);var numbers=mock(CaseNumberGenerator.class);when(numbers.next()).thenReturn("RS-2026-000001");service=new CaseService(repository,numbers,intake,Clock.fixed(now,ZoneOffset.UTC),mock(org.springframework.context.ApplicationEventPublisher.class));}
    ...
    @Test void submitsDraftAndCreatesReliableWorkflowEvents(){UUID id=UUID.randomUUID();var medicalCase=new MedicalCase(id,"RS-2026-000001","Jane Doe","Kenya","+254700000000",null,"en",now);when(repository.findById(id)).thenReturn(Optional.of(medicalCase));when(intake.onSubmitted(medicalCase)).thenReturn("safe-status-token");var response=service.submit(id);assertThat(response.status()).isEqualTo("RECEIVED");assertThat(response.statusToken()).isEqualTo("safe-status-token");verify(intake).validateSubmittable(id);verify(intake).onSubmitted(medicalCase);}
