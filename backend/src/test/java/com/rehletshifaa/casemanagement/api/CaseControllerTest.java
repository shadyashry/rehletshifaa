...
class CaseControllerTest {
    ...
    @Test void createsCaseFromMinimalValidPayload()throws Exception{UUID id=UUID.randomUUID();when(service.create(any())).thenReturn(new CaseDtos.CreateCaseResponse(id,"RS-2026-000001","DRAFT"));mvc.perform(post("/api/v1/cases").contentType(MediaType.APPLICATION_JSON).content("{\"givenName\":\"Jane\",\"familyName\":\"Doe\",\"country\":\"Kenya\",\"whatsappNumber\":\"+254700000000\",\"preferredLanguage\":\"en\",\"consent\":true}")).andExpect(status().isCreated()).andExpect(jsonPath("$.caseId").value(id.toString())).andExpect(jsonPath("$.status").value("DRAFT"));}
