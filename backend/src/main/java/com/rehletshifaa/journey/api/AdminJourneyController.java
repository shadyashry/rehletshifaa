...
@RestController @RequestMapping("/api/v1/admin") public class AdminJourneyController{
 ...
 @DeleteMapping("/practitioners/{id}/catalog/{serviceId}")public void deactivateService(@PathVariable UUID id,@PathVariable UUID serviceId){pricing.deactivateCatalogService(id,serviceId);}
 ...
 @PutMapping("/fx-rates/{currency}")public void setFxOverride(@PathVariable String currency,@Valid @RequestBody FxOverrideRequest request){pricing.setFxOverride(currency,request);}
}
