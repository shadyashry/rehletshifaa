...
public class CaseService {
    ...
    private final MedicalCaseRepository cases; private final CaseNumberGenerator numbers; private final NotificationPort notifications; private final Clock clock;
    public CaseService(MedicalCaseRepository cases, CaseNumberGenerator numbers, NotificationPort notifications, Clock clock) { this.cases=cases; this.numbers=numbers; this.notifications=notifications; this.clock=clock; }
    @Transactional public CreateCaseResponse create(CreateCaseRequest request) { Instant now = clock.instant(); var medicalCase = new MedicalCase(UUID.randomUUID(), numbers.next(), request.fullName(), request.country(), request.whatsappNumber(), request.conditionDescription(), request.preferredLanguage(), now); cases.save(medicalCase); return new CreateCaseResponse(medicalCase.getId(), medicalCase.getCaseNumber(), medicalCase.getStatus().name()); }
    @Transactional public SubmitCaseResponse submit(UUID id) { MedicalCase medicalCase = findDraft(id); try { medicalCase.submit(clock.instant()); } catch (IllegalStateException e) { throw new ApiException(409, "CASE_NOT_DRAFT", "Case cannot be submitted in its current state"); } cases.save(medicalCase); try { notifications.notifyNewCase(new NotificationPort.NewCaseNotification(medicalCase.getCaseNumber(), medicalCase.getFullName(), medicalCase.getCountry(), medicalCase.getWhatsappNumber())); } catch (RuntimeException e) { log.error("New-case notification delivery failed for an internal case", e); } return new SubmitCaseResponse(medicalCase.getCaseNumber(), medicalCase.getStatus().name()); }
