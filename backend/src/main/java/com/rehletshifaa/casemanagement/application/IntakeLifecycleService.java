...
public class IntakeLifecycleService {
    ...
    @Transactional public void createFoundation(MedicalCase medicalCase, CreateCaseRequest request) {
        Instant now=clock.instant(); UUID patientId=UUID.randomUUID(); UUID challengeId=UUID.randomUUID(); String code="%06d".formatted(random.nextInt(1_000_000));
        jdbc.sql("INSERT INTO patient_profiles(id,full_name,country,whatsapp_number,email,preferred_language,time_zone,created_at,updated_at,version) VALUES(?,?,?,?,?,?,?,?,?,0)")
            .params(patientId,request.fullName().trim(),request.country().trim(),request.whatsappNumber().trim(),blankToNull(request.email()),request.preferredLanguage(),blankToNull(request.timeZone()),now,now).update();
        jdbc.sql("UPDATE medical_cases SET patient_id=? WHERE id=?").params(patientId,medicalCase.getId()).update();
        jdbc.sql("INSERT INTO case_claim_challenges(id,case_id,patient_id,token_hash,delivery_channel,destination_hint,expires_at,attempts,max_attempts,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)")
            .params(challengeId,medicalCase.getId(),patientId,hash(code),"WHATSAPP",mask(request.whatsappNumber()),now.plusSeconds(expirySeconds),0,maxAttempts,now).update();
        ...
        jdbc.sql("INSERT INTO consent_records(id,patient_id,case_id,consent_type,policy_version,language,exact_text,purpose,scope,channel,captured_by,effective_from,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)")
            .params(UUID.randomUUID(),patientId,medicalCase.getId(),"DATA_PROCESSING","intake-v1",request.preferredLanguage(),consentText,"Medical case coordination","Submitted case data","WEB","guest",now,now).update();
        jdbc.sql("INSERT INTO notification_outbox(id,notification_type,channel,destination,template_key,template_data,status,attempts,max_attempts,next_attempt_at,idempotency_key,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)")
            .params(UUID.randomUUID(),"CASE_CLAIM","WHATSAPP",request.whatsappNumber().trim(),"case-claim-code","{\"code\":\""+code+"\"}","PENDING",0,5,now,"claim:"+challengeId,now).update();
    ...
    @Transactional public void onSubmitted(MedicalCase medicalCase) {
        ...
        jdbc.sql("INSERT INTO case_status_history(id,case_id,from_status,to_status,actor_subject,actor_role,reason,created_at) VALUES(?,?,?,?,?,?,?,?)")
            .params(UUID.randomUUID(),medicalCase.getId(),"DRAFT","RECEIVED","guest","GUEST","Patient submitted intake",now).update();
        jdbc.sql("INSERT INTO notification_outbox(id,notification_type,channel,destination,template_key,template_data,status,attempts,max_attempts,next_attempt_at,idempotency_key,created_at) SELECT ?,?,?,?,?,?,?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM notification_outbox WHERE idempotency_key=?)")
            .params(UUID.randomUUID(),"NEW_CASE","EMAIL",coordinatorEmail,"new-case-received","{}","PENDING",0,5,now,"case-submitted:"+medicalCase.getId(),now,"case-submitted:"+medicalCase.getId()).update();
